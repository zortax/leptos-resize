...
pub fn ResizableSplit(
    ...
    let col = col.unwrap_or_else(|| {
        ...
        create_rw_signal(vec![initial_size; num_children - 1])
    });
        ...
        if let (Some(index), Some(container)) = (resizing_index.get(), container.get()) {
            ...
            let new_percentages = match direction {
                SplitDirection::Row => {
                    ...
                    calculate_new_percentages(col.get(), index, x_percentage)
                }
                SplitDirection::Column => {
                    let y = ev.client_y() as f64 - bounds.top();
                    let y_percentage = (y / bounds.height()) * 100.0;
                    calculate_new_percentages(col.get(), index, y_percentage)
                }
            };
            col.set(new_percentages);
        }
    };
    ...
    let grid_template_columns = move || match direction {
        SplitDirection::Row => {
            ...
            for (i, percentage) in col.get().iter().enumerate() {
                grid_template.push_str(&format!("{:.2}% ", percentage));
                if i < col.get().len() - 1 {
                    grid_template.push_str(" ");
                }
            }
            grid_template.push_str(&format!(
                "{:.2}%",
                100.0 - col.get().iter().sum::<f64>()
            ));
            grid_template
        }
...
/// Calculates the new percentages for the splits based on the current mouse position.
